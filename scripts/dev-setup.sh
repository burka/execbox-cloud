...
final_setup() {
    ...
    echo "2. Start the backend server (runs on port 28080):"
    echo "   make run"
    echo ""
    echo "3. In another terminal, start the frontend (runs on port 25173):"
    echo "   make dev-frontend"
    echo ""
    echo "4. Open http://localhost:25173 in your browser"
    echo ""
    echo "💻 Development Port Layout:"
    echo "   • Database:   5433 (PostgreSQL on Docker)"
    echo "   • Backend:    28080 (Go API server)"
    echo "   • Frontend:   25173 (Vite dev server)"
    echo ""
